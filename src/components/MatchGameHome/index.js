...
class MatchGameHome extends Component {
  state = {
    initialId: tabsList[0].tabId,
    initialShuffleImage: imagesList[0],
    showImagesStatus: true,
    counter: 60,
    score: 0,
  }
  ...
  reset = () => {
    this.setState({showImagesStatus: true, counter: 60, score: 0})
    this.callTimer()
  }
    ...
    return (
      <div className="showScore-container">
        ...
        <div className="retry-container">
          ...
          />
          <button type="button" className="retry-button" onClick={this.reset}>
            PLAY AGAIN
          </button>
        </div>
      </div>
  ...
  showImages = () => {
    ...
    return (
      <div className="match-thumbnail-container">
        <img
          src={initialShuffleImage.imageUrl}
          alt="match"
        ...
        <ul className="tab-container">
          {tabsList.map(eachTab => (
            <TabItem
              tab={eachTab}
              key={eachTab.tabId}
              changeInitialId={this.changeInitialId}
              isActive={initialId === eachTab.tabId}
            />
          ))}
        </ul>
        <ul className="ul-thumbnail-container">
          {filterImages.map(eachOne => (
            <Thumbnail
              item={eachOne}
              key={eachOne.id}
  ...
  render() {
    const {showImagesStatus, counter, score} = this.state
    return (
      <div className="bg-container">
        <Navbar counter={counter} score={score} />
        {showImagesStatus ? this.showImages() : this.showScore()}
      </div>
    )
  }
}
